package serviciosrest.cliente.servicio;
...
public class ServicioProxyVideojuego {
	...
	//La URL base del servicio REST (servidor) de videojuegos
	public static final String URL = "http://localhost:8888/videojuegos/";
	 ...
	 * @return devuelve el videojuego que estamos buscando, null en caso de que el
	...
	public Videojuego obtener(int id){
		try {
			...
			//Ej http://localhost:8888/videojuegos/1 GET
	 ...
	 * id por lo que tiene que estar presente.
	...
	public List<Videojuego> listar(String nombre){
		...
		try {
			//Ej http://localhost:8888/videojuegos?nombre=nombreabuscar GET
			ResponseEntity<Videojuego[]> response =
					  restTemplate.getForEntity(URL + queryParams, Videojuego[].class);
			Videojuego[] arrayVideojuegos = response.getBody();//getBody() devuelve un array de objetos
